...
- **选题管线**：Polymarket Gamma API 拉活跃市场 → horizon 三档筛选 → LLM 译中文入题池
- **运维**：每日预测轮/揭晓轮/健康巡检三件套（Windows schtasks 编排示例见 `scripts/run_silent.py`）
...
├── scripts/             # 运维脚本（daily/evolve 轮、健康自检、dashboard、pm_fetch）
├── tests/               # 全量单测（pytest）
