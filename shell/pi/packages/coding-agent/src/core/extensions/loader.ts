...
async function loadExtensionModule(extensionPath: string, cacheToken?: ExtensionCacheToken) {
	...
	const jiti = createJiti(import.meta.url, {
		moduleCache: false,
		// Never cache on disk: jiti's fsCache does not invalidate on source mtime, so
		// edits to .foresight/extensions/*.ts would keep running stale compiled output
		// even across /reload and full restarts. (Foresight fork patch.)
		fsCache: false,
